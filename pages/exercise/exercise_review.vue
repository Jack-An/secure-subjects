...
<template>
	<view class="page_big" :style="{height:screenheight+'px'}">
		<u-navbar title="收藏练习" safeAreaInsetTop fixed placeholder :autoBack="true" :background="top_bgCl"
	...
	export default {
		...
		onLoad(option) {
			...
			if (option.questionbankId) {
				...
				this.getQuestionList(option.questionbankId,option.indexs);
			} else {
				uni.showToast({
					title: '数据加载失败！',
					icon: 'error'
		...
		onBackPress() {
			console.log("我是返回>>>>>>>", this.startTime);
			if (this.startTime > 0) {
		...
		methods: {
			...
			initExamNumData: function(questionbank) {
				...
				let radioNum = parseInt(questionbank.radioNum);
				let checkboxNum = parseInt(questionbank.checkboxNum);
				let judgeNum = parseInt(questionbank.judgeNum);
				let answerNum = parseInt(questionbank.answerNum);
				if (radioNum > 0) {
					this.setExamNumData(1, '单项选择题', radioNum);
				}
				if (checkboxNum > 0) {
					this.setExamNumData(2, '多项选择题', checkboxNum);
				}
				if (judgeNum > 0) {
					this.setExamNumData(3, '判断题', judgeNum);
				}
				if (answerNum > 0) {
					this.setExamNumData(4, '简答题', answerNum);
				}
			...
			getQuestionList: function(questionbankId,indexs) {
				
				uni.showLoading({
					title: '加载中....'
				});
				this.$Request.post('mainPath/appWebQuestion/favorites/getQuestionCollectList', {
					'queryFavorites.createUser':this.userId,
					'queryFavorites.questionbankId':questionbankId
				}).then(
					res => {
						uni.hideLoading();
						if (res.success) {
							...
							if(indexs){
								this.exampagenum=parseInt(indexs);
							}
			...
			changeOptions(index) {
				...
				if (!answerObj.isAnswered && answerObj.type == 1) {
					...
					if (answerObj.questionAnswerList[index].status == undefined || answerObj.questionAnswerList[index].status == '') {
						answerObj.questionAnswerList[index].status = 1;
