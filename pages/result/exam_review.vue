...
<template>
	<view class="page_big" :style="{height:screenheight+'px'}">
		<u-navbar :title="navbarTitle" safeAreaInsetTop fixed placeholder :autoBack="true" :background="top_bgCl"
			...
			<swiper style="height: 80%;" :indicator-dots="false" :circular="false" :autoplay="false" interval="5000"
				...
				<swiper-item v-for="(item, index) in answerData" :key="index">
					<view class="" style="height: 100%; overflow: scroll;">
						...
						<view class="analysis" v-if="item.type!=4">
							<view class="analysis_answer">
								...
								<view class="my_answer">
									...
									<view class="correct_answer_num" :style="{'color':item.isCorrect==1?'#10C600':'#E50000'}">
										{{item.myAnswer}}
									</view>
								</view>
							</view>
...
<script>
	export default {
		...
		onLoad(option) {
			this.answerData = [];
			//option.achievementId="1497482851241689088";
			//option.testpaperId="1492743566269612032";
			if (option.testpaperId) {
				this.getQuestionList(option);
			} else {
				uni.showToast({
					title: '数据加载失败！',
					icon: 'error'
		...
		methods: {
			...
			getQuestionList: function(options) {//.achievementId,option.testpaperId,option.indexs
				uni.showLoading({
					title: '加载中....'
				});
				let achievementId = options.achievementId;
				if(undefined == achievementId){
					achievementId = 0;
				}
				this.$Request.post('mainPath/appWebQuestion/testpaperAchievement/getTestpaperRecordList', {
					'testpaperAchievement.createUser':this.userId,
					'testpaperAchievement.id':achievementId,
					'testpaperAchievement.testpaperId':options.testpaperId,
				}).then(
					res => {
						uni.hideLoading();
						if (res.success) {
							this.answerData = res.entity.questionList;
							
							for (var i = 0; i < this.answerData.length; i++) {
								if(i==0){
									if(this.answerData[i].isCollect>0){
										this.collect=true;
									}else{
										this.collect=false;
									}
								}
								if (this.answerData[i].type == 3) {
									this.answerData[i].questionAnswerList = [{
										'answerTitle': 'A',
										'status': 0
									}, {
										'answerTitle': 'B',
								...
								this.changeOptions(i);
							}
							this.questionbankData = res.entity.questionbank;
							if(options.indexs){
								this.exampagenum=parseInt(options.indexs);
							}
							// this.practiceId=res.entity.practiceId;
							this.initExamNumData(this.questionbankData);
						}
					}).catch(err => {
					console.error(err);
					uni.hideLoading();
			...
			collectChange() {
				...
				this.$Request.post('mainPath/appWebQuestion/favorites/insetFavorites', {
					...
					res => {
						...
						if (res.success) {
							...
							this.$eventHub.$emit('shouchang',true);
						}
					}).catch(err => {
					console.error(err);
					uni.hideLoading();
			...
			//选择题答案改变样式
			changeOptions(index) {
				let answerObj = this.answerData[index];
				
				if (answerObj.type == 1||answerObj.type == 3) {
					for (let i = 0; i < answerObj.questionAnswerList.length; i++) {
						answerObj.questionAnswerList[i].status = 0;
						if(answerObj.isCorrect==2&&answerObj.myAnswer==answerObj.questionAnswerList[i].answerTitle){
				...
				} else if (answerObj.type == 2) {
					let rightkeyArr=answerObj.rightkey.split(',');
					for(let i = 0; i < answerObj.questionAnswerList.length; i++){
						answerObj.questionAnswerList[i].status = 0;
						if(answerObj.isCorrect==2){
							let myArr=answerObj.myAnswer.split(',');
							if(myArr.indexOf(answerObj.questionAnswerList[i].answerTitle)>-1){
								answerObj.questionAnswerList[i].status = 2;
							}
						}
						if(rightkeyArr.indexOf(answerObj.questionAnswerList[i].answerTitle)>-1){
							answerObj.questionAnswerList[i].status = 1;
						}
			...
			checkboxChangeOptions: function(index) {
				...
				if (!answerObj.isAnswered) {
					...
					if (answerObj.rightkey != myAnswer) {
						for (let i = 0; i < answerObj.questionAnswerList.length; i++) {
							if (answerObj.questionAnswerList[i].isCheck && answerObj.questionAnswerList[i].isCorrect == 2) {
								answerObj.questionAnswerList[i].status = 2;
							}
