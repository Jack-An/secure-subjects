<template>
	<view class="">
		<view class="u-navbar" :style="[navbarStyle]"
			...
			<view class="u-navbar-inner" :style="[navbarInnerStyle]">
				...
				<view class="u-navbar-content-title" v-if="title" :style="[titleStyle]">
					<view class="u-title" :style="{
							color: titleColor,
							fontSize: titleSize + 'rpx',
							fontWeight: titleBold ? 'bold' : 'normal'
						}" style="white-space: nowrap;" @click="pickShowfn">
						{{ title }}
						<!-- 增加下拉选择 -->
						<u-icon v-if="select" style="margin-left: 20rpx;" name="arrow-down" :color="titleColor"
							size="20"></u-icon>
						<!-- <u-icon v-if="select" style="margin-left: 20rpx;" name="arrow-down" :color="titleColor"
							size="20" @click="showChange"></u-icon> -->
					</view>
					<!-- 选择科目隐藏盒子 -->
					<view v-if="hidebox" class="hide_box">
						<view class="hide_box_item" v-for="(item,i) in changedata" :key="i" hover-class="hide_box_item_active" @click="ChangeSubject(item.changetitle)">
							{{item.changetitle}}
						</view>
					</view>
				</view>
		...
		<view class="u-navbar-placeholder" v-if="isFixed && !immersive"
			:style="{ width: '100%', height: Number(navbarHeight) + statusBarHeight+25 +'px'}"></view>
	</view>
</template>
